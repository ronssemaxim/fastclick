// -*- c-basic-offset: 4; related-file-name: "toovsdpdkring.hh" -*-
/*
 * fromovsdpdkring.{cc,hh} -- element that reads packets from a circular
 * ring buffer using DPDK and Open vSwitch.
 * Maxim Ronsse
 */
...
#include "fromovsdpdkring.hh"

CLICK_DECLS

FromOVSDPDKRing::FromOVSDPDKRing() :
...
int
FromOVSDPDKRing::configure(Vector<String> &conf, ErrorHandler *errh)
{
...
int
FromOVSDPDKRing::initialize(ErrorHandler *errh)
{
    if ( _burst_size == 0 ) {
    ...
    _recv_ring    = rte_ring_lookup   (_PROC.c_str());
    ...
    if ( !_recv_ring )
        return errh->error("[%s] Problem getting Rx ring with name %s. "
...
void
FromOVSDPDKRing::cleanup(CleanupStage)
{
}

bool
FromOVSDPDKRing::run_task(Task *t)
{
#if HAVE_BATCH
...
String
FromOVSDPDKRing::read_handler(Element *e, void *thunk)
{
    FromOVSDPDKRing *fr = static_cast<FromOVSDPDKRing*>(e);
...
EXPORT_ELEMENT(FromOVSDPDKRing)
ELEMENT_MT_SAFE(FromOVSDPDKRing)
